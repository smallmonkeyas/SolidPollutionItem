/*
 * @Author: your name
 * @Date: 2021-07-07 00:35:15
 * @LastEditTime: 2021-07-07 03:39:22
 * @LastEditors: Please set LastEditors
 * @Description: In User Settings Edit
 * @FilePath: \components\src\controls\dataAnamaly\anamalyDetectChartTab\anamalyDetectChartTab_actual157_v0707.js
 */
 //正式环境
...
class AnamalyDetectChartTab extends Component {
  ...
  getAnamalyCurveTable = selectedRows => {
    ...
    propHisPromise.then(resArr => {
      ...
      chart.xAxis[0].addPlotLine({
        // 在x轴上增加
        value: moment(selectedRows.starttime).valueOf(), // 在值为2的地方
        width: 2, // 标示线的宽度为2px
  ...
  componentDidMount = () => {
    ...
    if (oobj.starttime !== '') {
      ...
      console.log(
        'alarmStartTime',
        moment(this.alarmStartTime)
          .add(1, 'days')
          .format('YYYY-MM-DD HH:mm:ss'),
        this.alarmSourceName
      );
      this.endtime = moment(this.alarmStartTime)
        .add(1, 'days')
        .format('YYYY-MM-DD HH:mm:ss');
      ...
      this.setState({
        timeRangeValue: [
          moment(
            moment(this.alarmStartTime).subtract(12, 'days'),
            'YYYY-MM-DD HH:mm:ss'
          ),
          moment(
            moment(this.alarmStartTime).add(1, 'days'),
            'YYYY-MM-DD HH:mm:ss'
          ),
        ],
      });
