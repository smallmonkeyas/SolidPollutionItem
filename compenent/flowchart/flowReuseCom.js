  ...
  class CustomComp extends Component {
    ...
    componentDidMount = () => {
      ...
      factoryInfo.then(data => {
        ...
        // console.log(data,factoryCollector)
      })
      cookie = getCookie();
  ...
  */
  ...
  const getVendor = async function(urlpath){
    ...
    const response = await fetch("http://10.32.203.157:8999/serverapi/cfg/collector", requestOptions)
    const result = await response.text()
  ...
  const convertToObj = function(data,key,value){
    let obj={}
    data.forEach(item =>{
      let period = item[value].replace(/s/g,'秒').replace(/m/g,'分钟').replace(/h/g,'小时').replace(/d/g,'天')
      obj[item[key]] = period
    })
    return obj
  }